...
public class JavaFXUI extends GUIBase {
    ...
    public void drawTarget(Attacker attacker, Pencil pencil) {
        WayPoint2D target = attacker.target();
...
//                pencil.filled().color(color).circle(wayPoint2D.origin, wayPoint2D.radius);
    ...
    public void drawGrid(Attacker attacker, Pencil pencil) {
        Grid2D gridWorld = attacker.gridWorld();
        Path2D<Cell2D> areaPath = attacker.areaPath();
        if (gridWorld != null) {
    ...
    private Attacker leftUpAttacker() {
        ...
        WayPoint2D target = new WayPoint2D(targetPosition, 5, new Vector2(), 0);
