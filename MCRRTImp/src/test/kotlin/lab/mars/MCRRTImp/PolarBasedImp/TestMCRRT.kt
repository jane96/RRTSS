...
import lab.mars.MCRRTImp.base.WayPoint
...
class TestMCRRT {
    ...
    @Test
    fun testListMapSortedOrder() {
        val testCase = ArrayList<Vector2>()
        for (i in 0..99) {
            testCase.add(Vector2(0.0 random 100.0, 0.0 random 100.0))
        }
        val comparator = Vector2(1.0, 0.0)
        val map = testCase.map { dir -> Pair(dir, dir.angle(comparator)) }.sortedBy { it.second }
        map.forEach { e -> println(e.first + ", " + e.second) }
    }

    @Before
    fun buildWorld() {
        val circleObstacles = ArrayList<Obstacle<Vector2>>()
        val attackers = ArrayList<Attacker<Vector2>>()
        world = World(attackers, circleObstacles, Space(Vector2(2000.0, 2000.0), Vector2()))
    ...
    private fun leftUpAttacker(): Attacker<Vector2> {
        ...
        val targetPosition = Vector2(1900.0, 1900.0)
    ...
    @Test
    fun testAll() {
        world!!.attacker().forEach{ it.startAlgorithm() }
    }
}
